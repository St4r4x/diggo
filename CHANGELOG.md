...
### Fixed
- `dashboard/llm.py` — a provider name missing from `_PROVIDER_DEFAULTS` (future `_KNOWN_LLM_PROVIDERS` drift, or a stale DB row) raised a bare `KeyError` that `call_llm()`'s fallback loop swallowed as "provider failed, try the next one"; `_call_openai_compatible` now raises a distinct `ValueError` for this case, and the fallback loop re-raises it immediately instead of hiding it
- `dashboard/prepare_state.py` — `_run_prepare` showed the generic "add a provider" error when every configured provider's key failed to decrypt (e.g. after a `SECRET_KEY` rotation), even though the user did configure one; now shows a distinct "your key(s) could not be decrypted, re-save it" message when providers were configured but none decrypted
- `dashboard/api.py` — `PUT /api/settings/llm-providers/{provider}` raised an unhandled `AttributeError`/500 when `api_key` was a non-string JSON value (`null`, a number, etc.); now returns a 422 with `{"error": "invalid_request", ...}`
...
- `tests/test_auth.py` — renamed the leftover `"hf_token_complete"` key in 7 mocked onboarding-state fixtures to `"llm_provider_complete"`, matching the multi-provider rename; the key was inert (only `is_complete`/`profile_complete` are read) but was exactly the kind of stale reference the final verification grep check is meant to catch
...
- `frontend/components/settings/llm-providers-section.tsx` — `deleteMutation` and `reorderMutation` had no `onError` handler, so a failed delete/reorder request looked like nothing happened; delete errors now surface inline on the affected row (via `rowError`, same as `saveMutation`), reorder errors show as a line above the provider list. Also, `ProviderRow`'s submit handler cleared the API key input synchronously on submit regardless of outcome, so a rejected (422) save already showed a blank input; the input is now cleared only after `saveMutation`'s `onSuccess` actually fires for that row (via a `savedNonce` counter bumped per-provider)

### Added
- `alembic/versions/0007_llm_providers.py`, `dashboard/user_data.py` — `user_llm_providers` table replacing the single `hf_token_encrypted` column; each user can now configure Hugging Face, Ollama Cloud, OpenAI, Anthropic, and Groq keys with a reorderable try-order, backed by `get_llm_providers()`, `get_llm_provider_key()`, `save_llm_provider()`, `delete_llm_provider()`, and `reorder_llm_providers()`
