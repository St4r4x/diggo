...
Jinja2 routes deleted at the end of this sub-phase (once nginx stops routing `/candidatures` to `api`): `GET /candidatures` (`index()`), `GET /offers` (`offer_list()`), `GET /offers/{offer_id}` (`offer_detail()`) in `dashboard/app.py`, plus `dashboard/templates/index.html` and `dashboard/templates/partials/offer_list.html`/`offer_detail.html`. `GET /offers/{offer_id}/edit` (the edit form route) is **not** deleted yet — it's still reachable and will be migrated in sub-phase B.
